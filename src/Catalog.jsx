...
import AllRentalLogo from './components/AllRentalLogo.jsx'
...
/* ============ 인터랙티브 렌탈료 계산기 ============ */
function Calculator({ matrix, colors = [], onPick, discount = 0, setDiscount }) {
  const avail = useMemo(() => {
...
function DetailSection({ p, scrollRef }) {
  if (!p) return null
              ...
              <div className="info-block">
                ...
                <div className={`table-scroll ${tableOpen ? '' : 'collapsed'}`}>
                  <table className="matrix-table">
                    <thead>
                      <tr><th>관리방법</th><th>관리주기</th><th>약정기간</th><th>월 렌탈료</th></tr>
                    </thead>
                    <tbody>
                      {matrix.map((r, i) => (
                        <tr key={i}>
          ...
          <div className="modal-veil" ref={veilRef}
            ...
            <div className="modal-card fullscreen">
              ...
              <div className="modal-body" ref={modalBodyRef}>
                <DetailSection p={sel} scrollRef={modalBodyRef} />
              </div>
            </div>
          </div>
        )}
